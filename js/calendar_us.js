...
var A_TCALDEF = {
	...
	'imgpath' : 'images/imgcal/' // directory with calendar images
}
var A_TCALSIDX = {
		'months' : ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
		'weekdays' : ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'],
...
function f_tcalParseDate (s_date) {
	...
	if (!re_date.exec(s_date))
		return alert ("Invalid date, Please enter the date in DD/MM/YYYY format.")
	var n_day = Number(RegExp.$1),
		n_month = Number(RegExp.$2),
		n_year = Number(RegExp.$3);
	if (!re_Year.exec(n_year))
		return alert ("Invalid Year, Please enter the date in DD/MM/YYYY format.")
	if (n_year < 100)
...
function f_tcalHide (n_date) {
	...
	// Code added -- Balaji 24 Jun 10
    if (document.createEventObject){
        // dispatch for IE
        var evt = document.createEventObject();
        this.e_input.fireEvent('onchange',evt);
    } else{
        // dispatch for firefox + others
        var evt = document.createEvent("HTMLEvents");
        evt.initEvent('change', true, false ); // event type,bubbling,cancelable
        var el =document.getElementById(this.a_cfg.controlname);
        //var canceled = !this.e_input.dispatchEvent(evt);
